...
## 共通
電気分野の高橋開人、電子分野の水野拓真、機械分野の水野俊太、情報科の小松迅人多田隆人、吉田瞬吏です。  
よろしくお願いします。
```
- 科学技術科
  - 電気分野 高橋開人
...
### はじめに
近い将来に完全な自動運転が始まり、その後は全ての車が自動運転されると思います。  
私たちはそんな未来での都市について5分野で協力し、3チームに別れて研究しようと思いました。  
...
を担当することにしました。  
今回の発表では `機体チーム` と `情報科チーム` の発表を合同で行います。  
...
## 電気/電子/機械分野
### はじめに
`機体チーム` は 自動給電自動走行とそれを実現するための車体の製作を担当しました。  
電気自動車と自動走行の問題点として、  
充電する際に時間がかかってしまう点と  
気候などの条件により画像認識の精度が落ちるという点  
があると考えました。  
...
概要としては自動走行中に交差点等の地面に埋め込まれているNFCタグをRFIDセンサーで検知し、待機、直進、左折、右折をさせます。

私の当初の目標は地面に埋め込まれた磁石を用いた磁気トレースで自動走行をさせようとしていました。
しかし、磁気センサーには2つの問題点がありました。
- １つ目は磁気センサーを使った時にREIDリーダーが正常に動作しないこと
- 2つ目は無線自動給電から影響を受けることです。

このことから、目標を赤外線反射センサーを用いたライントレースでの自動走行に変更しました。  
今回は自動走行の制御にPID制御を使用しました。

ライントレースをしながらNFCタグを検知しその場で情報チームの最適経路のシステムと通信し、  
車にどのような動作をさせるかの指令を送り実行する予定でしたが、  
情報科と合わせる前に元からNFCタグに情報を入れておき、簡易的に動かしてみました。

試走実験をする床によってNFCタグが反応しない事があったので原因を調べた結果タグが反応しない床には  
鉄が埋め込まれている事が分かりました。  
その事から、このシステムを導入するにあたって水道管などの地中の鉄にも影響を受けてしまうのではないかと考えました。  

今後の課題としては、地中に鉄があっても支障なくシステムが動作できるようにすることです。  
また、情報科と合わせて取得した最短経路を正しく辿れるようにしたいです。
...
**0．155V** 電圧上がり少量ではありますが、バッテリーへの充電ができていることが確認できました。

実験でバッテリーに流れる電流の大きさがあまりにも小さすぎました。  
...
### 問題提起・目標
続いて情報科学科です。  
この機体を制御するにはサーバー側で処理する必要があり、情報科はこれを制作します。  
...
システム構成はこのようになっています。  
各クライアントとサーバーはsocket通信で接続されています。  
コントローラーを介して制御されています。  
データベースにはクライアント接続状況とタグの情報が保存されています。
...
Astar はゴールまでの直線距離も計算し、ゴール側に寄って走査するため効率よく探索できます。
...
放置するだけで様々な条件下でシミュレートできるようにもなっています。

こちらはシミュレーターを実行している様子です。  
iMac6台でそれぞれ9個、計54個回しています。
